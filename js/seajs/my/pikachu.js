    ...
    var pikachu = {
        ...
        mouseWheel: function () {
            $('.mainContent').mousewheel(function (event, delta) {
                ...
                delta > 0 ? arrIndex-- : arrIndex++;

                // arrIndex 有下限0, 上限跳转到另一个状态
                var pikachu_exam = require('./pikachu_exam');
                arrIndex = arrIndex < 0 ? 0 : arrIndex;
                if (arrIndex > 9){
                    // 把自己隐藏起来
                    // 隐藏权归自己，本函数在自己页面，隐藏后要重置状态变量
                    // 隐藏函数里会记录当前变化值
                    pikachu.classHideAndExamShow();
                }
        ...
        // 课时级隐藏，评测级显示
        classHideAndExamShow:function(){
            // 隐藏归自己控制，所以本函数写到 pikachu 页面
